			...
			for (Map.Entry<String, Object> entry : fetchData.entrySet()) {
				...
				try {
					...
					if (emailCred.isEmpty() && status == false) {
						loginPage.passWord(passCred);
						loginPage.clickloginButton();
						Assert.assertEquals(loginPage.EmailValidation.getText().toString(), messageData);
					...
					if (passCred.isEmpty() && status == false) {
						loginPage.loginCredential(emailCred);
						loginPage.clickloginButton();
						Assert.assertEquals(loginPage.passwordElementValidation.getText().toString(), messageData);
					...
					if (!passCred.isEmpty() && !emailCred.isEmpty() && status == false) {
						...
						loginPage.passWord(passCred);
						loginPage.clickloginButton();
						Assert.assertEquals(loginPage.invaidCredElementValidation.getText(), messageData);
